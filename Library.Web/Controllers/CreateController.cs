...
namespace Library.Web.Controllers
{
    public class CreateController : Controller
    {
        ...
        public async Task<IActionResult> Submit(string EmployeeLN, string EmployeeFN, string EmployeeID, string KNNR, string sort, string PrNr)
        {
            try
            {
                await RequestHandler.MakeRequest<List<CardsResource>>($@"{_environmentConfiguration.LibraryWebApiServiceHost}/api/library/Create/"+EmployeeLN+"/"+EmployeeFN+"/"+EmployeeID+"/"+KNNR+"/"+sort+"/"+PrNr);
            }
            catch
            {
